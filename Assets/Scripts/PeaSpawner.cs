...
public class PeaSpawner : MonoBehaviour
{
    ...
    [SerializeField] GameObject mystery;
    [SerializeField] int mysteryCount = 5;
    ...
    IEnumerator SpawnPeas()
    {
        ...
        while (spawned < spawnCount)
        {
            for (int i = 0; i < batchSize; i++)
            {
                ...
                GameObject toSpawn = pea;
                if (mysterySpawned < mysteryCount && Random.Range(0, spawnCount) < mysteryCount)
                {
                    toSpawn = mystery;
                    mysterySpawned++;
                }
